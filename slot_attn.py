...
from qasa_utils import gru_cell, linear
...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, slots_init, skip_norm = False):
        # `inputs` has shape [batch_size, num_inputs, input_size].
        # `slots` has shape [batch_size, num_slots, slot_size].
        slots = slots_init
        B, N_kv, D_inp = inputs.size()  # B 595 768
        B, N_q, D_slot = slots.size()  # B 7 256
...
class SlotAttentionEncoder(nn.Module):
    ...
    def forward(self, x, skip_norm=False):
        ...
        slots, attn, attn_logits = self.slot_attention(x, init_slots, skip_norm)
        # `slots` has shape: [batch_size, num_slots, slot_size].
