...
class PascalVOC(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split=='trainaug':
            ...
            return img_id, img, mask_instance, mask_class, ignore_mask
...
class COCO2017(Dataset):
    NUM_CLASSES = 81  # background ID 0
 ...
 89, 90]
    ...
    def __init__(self, root, livis = False, split='train', year='2017', image_size=224, mask_size=224, return_gt_in_train=False):
        super().__init__()
        ...
        if split == "train+unlabeled":
            ann_file = os.path.join(root, 'annotations/instances_{}{}.json'.format('train', year))
            self.coco = COCO(ann_file)
            self.imgs.update(self.coco.imgs)
            ann_file_un = os.path.join(root, 'annotations/image_info_{}{}.json'.format('unlabeled', year))
            self.coco_un = COCO(ann_file_un)
            self.ids = list(self.coco.imgs.keys()) + list(self.coco_un.imgs.keys())
            self.imgs.update(self.coco_un.imgs)
        else:
            if livis:
                ann_file = os.path.join(root, 'annotations/instances_coco+lvis_{}{}.json'.format(split, year))
            else:
    ...
    def get_original_size(self, img_id):
        return (self.imgs[img_id]['height'], self.imgs[img_id]['width'])

    def get_original_image(self, img_id):
    ...
    def __getitem__(self, index):
        if self.split == "train+unlabeled":
            ...
            img_metadata = self.loadImgs(img_id)[0]
        ...
        img_id, img, mask_instance, mask_class, mask_ignore = self._make_img_gt_point_pair(index)
        ...
        if self.split == "train" and (self.return_gt_in_train is False):
            ...
            return img_id, img, mask_instance, mask_class, mask_ignore
        else:
            raise
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        return img_id, _img, mask_instance, mask_class, mask_ignore
